---
description: "Generate a lean, problem-first PRD (docs/PRD.md), then hand off to Plan mode for implementation planning."
argument-hint: "[product/feature idea] (blank = start with questions)"
origin: "ECC /plan-prd (seeded, repo-local; output → docs/PRD.md, handoff → Plan mode)"
---

# PRD Command

Produces a **Product Requirements Document** — the requirements-phase artifact of the SDLC. Captures *what* must be true for success and *why*, and stops before *how*. Implementation decomposition is delegated to **Plan mode**.
...
| Write `docs/PRD.md` | Produce an implementation plan — that's Plan mode |

If you find yourself writing implementation detail, stop and cut it. It belongs in the planning step.
...
> 참고: 이 프로젝트의 주제는 "하네스 구축"이고 파일 업로드는 적용 도메인이다(ADR-0001).
> PRD도 *무엇을 하네스로 만들어 무엇에 적용하는가* 관점을 반영한다.
...
Write the PRD to `docs/PRD.md` and report.
...
| # | Milestone | Outcome | Status | Devlog |
|---|---|---|---|---|
...
Next step: Plan 모드로 다음 pending 마일스톤의 구현 계획을 세운다.
```

## Integration

- **Plan mode** — consume the PRD and produce an implementation plan for the next pending milestone.
- `springboot-tdd` skill — implement the plan test-first.
- `create-git-repo` 스킬 / `.claude/CLAUDE.md` 커밋 규칙 — 단계별 커밋·푸시.
...
- **NO_IMPLEMENTATION_DETAIL**: file paths, libraries, or task breakdowns are absent — if they appeared, move them to the planning step.
